export default {
  eyebrow: "Contact us",
  title: "Let's join hearts",
  description:
    "Want to donate, volunteer, or get your company involved? Write to us — every message can help warm one more child.",
  note: "We're also interested in collaborative projects and sustainable, socially responsible tourism in the high-Andean communities.",
  labelName: "Name",
  placeholderName: "Your name",
  labelEmail: "Email",
  placeholderEmail: "you@email.com",
  labelPhone: "Phone",
  placeholderPhone: "+51",
  labelMessage: "Message",
  placeholderMessage: "Tell us how you want to join",
  termsBefore: "I have read and accept the",
  privacyLink: "privacy policy",
  submit: "Send message",
  sending: "Sending...",
  successTitle: "Message sent!",
  success: "Thanks for reaching out. Our team will get back to you very soon.",
  modalClose: "Close",
  error: "Couldn't send. Please try again.",
  errName: "Enter your name (min. 2 characters).",
  errEmail: "Enter a valid email.",
  errMessage: "The message must be at least 10 characters.",
  errTerms: "You must accept the privacy policy.",
} as const;
